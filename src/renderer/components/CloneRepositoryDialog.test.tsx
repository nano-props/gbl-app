...
import { CloneRepositoryDialog } from '#/renderer/components/CloneRepositoryDialog.tsx'
...
describe('CloneRepositoryDialog', () => {
  ...
  test('cancel aborts an in-flight clone and closes the dialog', async () => {
    ...
    expect(onClose).toHaveBeenCalledTimes(1)
    expect(
      rpcCalls.some((call) => call.path === 'repo.abortClone' && typeof (call.input as { operationId?: unknown })?.operationId === 'string'),
    ).toBe(true)
