...
import { ellipsizeLeftPathByWidth } from '#/renderer/lib/display-path.ts'
...
describe('FilePathText', () => {
  ...
  test('measures actual rendered width instead of estimating by character count', async () => {
    ...
    try {
      ...
      expect(span?.textContent).toBe(ellipsizeLeftPathByWidth(path, 112, (text) => measureTextWidth(text) + Math.max(0, text.length - 1)))
